...
docker build -t gcr.io/YOUR_PROJECT_ID/l0181:latest .

# Push to Container Registry
docker push gcr.io/YOUR_PROJECT_ID/l0181:latest

# Deploy to Cloud Run
gcloud run deploy l0181 \
  --image gcr.io/YOUR_PROJECT_ID/l0181:latest \
  --platform managed \
  --region us-central1 \
  --port 50181 \
...
- `PORT`: Automatically set by Cloud Run (the app defaults to 50181)
...
```bash
gcloud run services update l0181 \
  --region us-central1 \
...
gcloud run services update-traffic l0181 \
  --region us-central1 \
  --to-latest
...
1. **Port Configuration**: Ensure the app uses `process.env.PORT` and falls back to 50181
2. **Memory Limits**: Cloud Run defaults to 256MB. Increase if needed:
   ```bash
   gcloud run services update l0181 --memory 512Mi --region us-central1
   ```
3. **Cold Starts**: Cloud Run may have cold starts. Consider minimum instances:
   ```bash
   gcloud run services update l0181 --min-instances 1 --region us-central1
   ```
...
```bash
# List revisions
gcloud run revisions list --service l0181 --region us-central1
